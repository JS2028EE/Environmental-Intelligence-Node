...
static void drawMotion(){header("IMU MOTION");display.setCursor(0,16);display.printf("A %.1f %.1f %.1f",sensors.accelX,sensors.accelY,sensors.accelZ);display.setCursor(0,28);display.printf("MAG %.1f m/s2",sensors.accelMagnitude);display.setCursor(0,40);display.printf("TILT %.1f deg",sensors.tiltDegrees);display.setCursor(0,52);display.printf("%s%s",sensors.fallDetected?"FALL EVENT":sensors.motionState,sensors.tiltDetected&&!sensors.fallDetected?" TILT":"");}
static void drawHardware(){header("HARDWARE");display.setCursor(0,18);display.println("ESP32 + SSD1306");display.setCursor(0,32);display.println("MPU-9250 FAMILY IMU");display.setCursor(0,46);display.println("BREADBOARD PROTOTYPE");}
static void drawSensorStatus(){header("SENSOR STATUS");display.setCursor(0,16);display.printf("DHT11 %s",!isnan(sensors.temperatureC)?"OK":"--");display.setCursor(0,28);display.printf("IMU %s",sensors.mpuPresent?"OK":"--");}
static void drawAbout(){header("ABOUT");display.setCursor(0,18);display.println("VIGIL-01");display.setCursor(0,30);display.println("ENV INTELLIGENCE NODE");display.setCursor(0,44);display.println("MPU-9250 FAMILY IMU");}
...
void displayBegin(){
  // Wire is initialized once by sensorsBegin() because the OLED and motion IMU
  // share the same I2C bus. Reinitializing Wire here can disturb the bus.
