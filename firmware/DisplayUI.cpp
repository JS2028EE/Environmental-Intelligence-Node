...
static void drawMotion(){header("MPU6050 MOTION");display.setCursor(0,16);display.printf("A %.1f %.1f %.1f",sensors.accelX,sensors.accelY,sensors.accelZ);display.setCursor(0,28);display.printf("MAG %.1f m/s2",sensors.accelMagnitude);display.setCursor(0,40);display.printf("TILT %.1f deg",sensors.tiltDegrees);display.setCursor(0,52);display.printf("%s%s",sensors.impactDetected?"IMPACT":(sensors.motionDetected?"MOTION":"STABLE"),sensors.tiltDetected?" TILT":"");}
static void drawHardware(){header("HARDWARE");display.setCursor(0,18);display.println("ESP32 + SSD1306");display.setCursor(0,32);display.println("MPU6050 MOTION");display.setCursor(0,46);display.println("BREADBOARD PROTOTYPE");}
static void drawSensorStatus(){header("SENSOR STATUS");display.setCursor(0,16);display.printf("DHT11 %s",!isnan(sensors.temperatureC)?"OK":"--");display.setCursor(0,28);display.printf("MPU6050 %s",sensors.mpuPresent?"OK":"--");}
static void drawAbout(){header("ABOUT");display.setCursor(0,18);display.println("VIGIL-01");display.setCursor(0,30);display.println("ENV INTELLIGENCE NODE");display.setCursor(0,44);display.println("MPU6050 MOTION");}
...
void displayBegin(){
  // Wire is initialized once by sensorsBegin() because the OLED and GY-521
  // share the same I2C bus. Reinitializing Wire here can disturb an already-
  // initialized MPU6050, so the display only attaches to the existing bus.
  if(!display.begin(SSD1306_SWITCHCAPVCC,OLED_ADDRESS)){Serial.println("OLED FAILED");while(true)delay(1000);}display.setTextColor(SSD1306_WHITE);
}
