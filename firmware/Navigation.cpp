...
// Remembers the last cursor position for every screen, forever — the same
// behavior V1.1 got "for free" from having a separate persistent cursor
// global per menu, generalized here to any number of menus.
...
void navigationPoll() {
  if (buttonPressed(0)) navUp();
  if (buttonPressed(1)) navDown();
  if (buttonPressed(2)) navSelect();
  if (buttonPressed(3)) navBack();
}

static void beep(int freq, int ms) {
  if (settings.buzzerEnabled) tone(PIN_BUZZER, freq, ms);
}
...
static uint8_t itemCountFor(ScreenState s) {
  ...
  const Menu* m = getMenu(s);
  return m ? m->count : 0;
}

// Every screen has exactly one fixed parent. Derived by searching the menu
// table itself, so Back navigation can never drift out of sync with it —
// V1.1's goBack() was a hand-maintained switch that had to be kept in sync
// with selectCurrent() by hand.
static ScreenState parentOf(ScreenState s) {
  ...
  for (ScreenState menuId : menus) {
    ...
    for (uint8_t i = 0; i < m->count; i++) {
      if (m->items[i].target == s) return menuId;
    }
  }
  return HOME; // unreachable in practice; keeps the compiler happy
}

static void enterScreen(ScreenState target) {
  currentScreen = target;
  cursor = menuCursorMemory[target];
}
...
static void toggleSetting(uint8_t index) {
  switch (index) {
    ...
    case 1:
      ...
      break;
    case 2:
      settings.automaticAlerts = !settings.automaticAlerts;
...
static void navUp() {
  beep(950, 25);
  uint8_t count = itemCountFor(currentScreen);
  if (count == 0) return;
  cursor = (cursor + count - 1) % count;
  menuCursorMemory[currentScreen] = cursor;
}
...
static void navSelect() {
  beep(1200, 40);
  if (currentScreen == HOME) { enterScreen(MAIN_MENU); return; }
  if (currentScreen == SETTINGS_SCREEN) { toggleSetting(cursor); return; }
  const Menu* m = getMenu(currentScreen);
  if (m && cursor < m->count) enterScreen(m->items[cursor].target);
}

static void navBack() {
  beep(800, 30);
  currentScreen = parentOf(currentScreen);
  cursor = menuCursorMemory[currentScreen];
}
